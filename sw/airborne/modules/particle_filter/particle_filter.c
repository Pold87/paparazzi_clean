...
void read_measurements_from_csv(struct measurement zs[], char filename[], int size) {
  ...
  r = fscanf(fp, "%s\n", header);
  if (r == EOF)
    printf("[read_measurements_from_csv] Could not parse header");

  double dummy_num;
  for (i = 0; i < size; i++) {
    r = fscanf(fp, "%lf,%lf,%lf,%lf\n", &dummy_num, &zs[i].x, &zs[i].y, &zs[i].uncertainty);
    if (r == EOF)
      printf("[read_measurements_from_csv] Could not parse line");
  }
...
void read_predictions_from_csv(struct measurement zs[], char filename[], int size, int use_variance) {
  ...
  if (r == EOF)
    printf("[read_predictions_from_csv] Could not parse line");
...
void read_sift_from_csv(struct sift sifts[], char filename[]) {
  ...
  r = fscanf(fp, "%s\n", header);
  if (r == EOF)
    printf("[read_sift_from_csv] Could not parse header");

  
  double dummy_num;
  for (i = 0; i < M; i++) {
    r = fscanf(fp, "%lf,%lf,%lf,%d\n", &dummy_num, &sifts[i].x, &sifts[i].y, &sifts[i].matches);
    if (r == EOF)
      printf("[read_sift_from_csv] Could not parse line");
